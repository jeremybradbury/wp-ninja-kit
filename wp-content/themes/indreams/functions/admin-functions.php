...
function of_option_setup() {
    ...
    $template = indreams_get_option('of_template');
    $saved_options = indreams_get_option('of_options');
    $std = '';
    foreach ($template as $option) {
        if ($option['type'] != 'heading') {
            ...
            $db_option = indreams_get_option($id);
            if (empty($db_option)) {
                if (is_array($option['type'])) {
                    foreach ($option['type'] as $child) {
                        $c_id = $child['id'];
                        $c_std = $child['std'];
                        indreams_update_option($c_id, $c_std);
                        $of_array[$c_id] = $c_std;
                    }
                } else {
                    indreams_update_option($id, $std);
                    $of_array[$id] = $std;
                }
    ...
    indreams_update_option('of_options', $of_array);
}
