...
function indreams_optionsframework_add_admin() {
    ...
    $shortname = indreams_get_option('of_shortname');
    ...
    if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'optionsframework') {
        if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
            ...
            indreams_reset_options($options, 'optionsframework');
    ...
    $of_page = add_theme_page($themename, 'Theme Options', 'edit_theme_options', 'optionsframework', 'indreams_optionsframework_options_page', 'div');

    // Add framework functionaily to the head individually
    add_action("admin_print_scripts-$of_page", 'indreams_load_only');
}

add_action('admin_menu', 'indreams_optionsframework_add_admin');
/* ----------------------------------------------------------------------------------- */
...
function indreams_reset_options($options, $page = '') {
    ...
    if ($page == 'optionsframework') {
        indreams_delete_option('of_options');
    }
}
...
function indreams_optionsframework_options_page() {
    ...
    $return = indreams_optionsframework_machine($options);
    ?>
            <div id="main">
                <div id="of-nav">
                    <ul>
...
/* ----------------------------------------------------------------------------------- */
/* Ajax Save Action - indreams_ajax_callback */
/* ----------------------------------------------------------------------------------- */
...
function indreams_ajax_callback() {
    ...
    if ($save_type == 'upload') {
        ...
        indreams_update_option($clickedID, $uploaded_file['url']);
        ...
        foreach ($options as $option_array) {
            $id = $option_array['id'];
            $old_value = indreams_get_option($id);
            $new_value = '';
            ...
            if (isset($option_array['id'])) { // Non - Headings...
                ...
                if (is_array($type)) {
                    ...
                    indreams_update_option($id, 'false');
                } elseif ($new_value == 'true' && $type == 'checkbox') { // Checkbox Save
                    indreams_update_option($id, 'true');
                    ...
                    foreach ($option_options as $options_id => $options_value) {
                        ...
                        if (!isset($output[$multicheck_id])) {
                            indreams_update_option($multicheck_id, 'false');
                        } else {
                            indreams_update_option($multicheck_id, 'true');
                        }
                    }
    ...
    foreach ($options as $value) {
        ...
        switch ($value['type']) {
            ...
            case 'select':
                ...
                $select_value = indreams_get_option($value['id']);
            ...
            case "upload":
                $value['std'] = '';
                if (isset($value['std'])) {
                    $output .= indreams_optionsframework_uploader_function($value['id'], $value['std'], null);
                }
                break;
            case "upload_min":
